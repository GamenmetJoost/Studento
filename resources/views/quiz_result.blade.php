<x-app-layout>
    ...
    <div class="mb-8 p-6 rounded-lg bg-white dark:bg-gray-800 shadow flex flex-col md:flex-row md:items-center md:justify-between gap-4 border border-gray-200 dark:border-gray-700">
            <div>
                <p class="text-xl font-semibold text-gray-800 dark:text-gray-100">Score</p>
                <p class="text-4xl font-bold mt-1 text-emerald-600 dark:text-emerald-400">{{ $score }} / {{ $total }}</p>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ number_format(($score / max(1,$total)) * 100, 1) }}% juist</p>
            </div>
            ...
            <div class="flex gap-3">
                <a href="{{ route('toetsen.category', ['category_id' => $category->id]) }}" class="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600 border border-transparent dark:border-gray-600">Opnieuw maken</a>
        ...
        <div class="space-y-6">
            @foreach($review as $item)
                <div class="p-5 rounded-lg bg-white dark:bg-gray-800 shadow border {{ $item['user_answer_is_correct'] ? 'border-emerald-300 dark:border-emerald-600' : 'border-red-300 dark:border-red-600' }}">
                    <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="space-y-1 text-sm">
                                @if($item['fallback_boolean'])
                                    ...
                                    <div class="mt-2 text-sm text-gray-700 dark:text-gray-300">
                                        Juiste antwoord:
                                        <span class="inline-block px-2 py-0.5 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
                                    ...
                                    @foreach($item['choices'] as $choice)
                                        <div class="flex items-center gap-2 text-gray-800 dark:text-gray-200">
                                            ...
                                            @if($choice['is_correct'])
                                                <span class="text-emerald-700 dark:text-emerald-400 text-xs font-medium">JUIST</span>
                                            @endif
                                            @if(!$choice['is_correct'] && $choice['is_selected'])
                                                <span class="text-red-600 dark:text-red-400 text-xs font-medium">GEKOZEN</span>
                                            @endif
                                        </div>
                                    @endforeach
                                    <div class="mt-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <div>
                            @if($item['user_answer_is_correct'])
                                <span class="inline-block px-3 py-1 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 text-xs font-semibold">Goed</span>
                            @else
                                <span class="inline-block px-3 py-1 rounded bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 text-xs font-semibold">Fout</span>
                            @endif
                        </div>
                    </div>
                </div>
